...
int Thread::Start(Process *owner, int32_t func, int arg)
{
    ...
    //on recupere le processus

    process = owner;

    //allocation de la pile pour l'utilisateur
    int32_t stackpointer = owner->addrspace->StackAllocate();

    //allocation de la pile pour le simulateur
    int8_t * pointeursimulateur = AllocBoundedArray(SIMULATORSTACKSIZE);

    //Initialisation du contexte du thread
    InitThreadContext(func,stackpointer,arg);

    //Initialisation du contexte du simulateur
    InitSimulatorContext(pointeursimulateur, SIMULATORSTACKSIZE);
    //On interrompt les interruptions pour eviter les accès aux processus
    IntStatus protection = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
    //On ajoute le thread à la liste de Thread du processus owner
    process->numThreads++;
    //on réactive
    g_machine->interrupt->SetStatus(protection);
    //ajoute dans la liste des threads
    g_alive->Append(this);
    //pour gerer les threads en attente/qui sont prets
    g_scheduler->ReadyToRun(this);
    //pour gerer l'erreur
    //g_syscall_error->SetMsg((char *)"Fin du start : ", NO_ERROR);
...
{
  #ifdef ETUDIANTS_TP
  //passe le thread en mode à détruire
  g_thread_to_be_destroyed=this;

  //On interrompt les interruptions pour eviter les accès aux processus
  IntStatus protection = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);
  //détruire le thread This qu'il soit pret, bloqué ou actif
