...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
 ...
 //***********************************************************************Exceptions ETUDIANTS_TP**************************************************************
  ...
  case SC_P :
    ...
    SemId idSemaP;
    idSemaP=(SemId)g_machine->ReadIntRegister(4);
    //on stocke notre objet quand on le cherche, on recupere le semaphore de g_object_ids
    Semaphore * pointeurDeSemaphoreP;
    pointeurDeSemaphoreP = (Semaphore *)g_object_ids->SearchObject(idSemaP);

    //si l'objet correspondant à idSema est de type semaphore
    if(pointeurDeSemaphoreP->type == SEMAPHORE_TYPE){
        //appel P sur le semaphore
        pointeurDeSemaphoreP->P();
	...
	case SC_V :
		...
		//lecture parametre dans registre r4
		SemId idSemaV;
    idSemaV=(SemId)g_machine->ReadIntRegister(4);
    //on stocke notre objet quand on le cherche, on recupere le semaphore de g_object_ids
    Semaphore * pointeurDeSemaphoreV;
    pointeurDeSemaphoreV = (Semaphore *)g_object_ids->SearchObject(idSemaV);

		//si l'objet correspondant à idSema est de type semaphore
		if(pointeurDeSemaphoreV->type == SEMAPHORE_TYPE){
        //appel V sur le semaphore
        pointeurDeSemaphoreV->V();
  ...
  case SC_SEM_CREATE:
    ...
    //recupere le debug_name et le count dans registre r4 et r5
		int32_t reg_debug_name;
    reg_debug_name=g_machine->ReadIntRegister(4);
    int32_t reg_count;
    reg_count=g_machine->ReadIntRegister(5);
  ...
  //recupere le sema r4
  SemId reg_sema;
  reg_sema=(SemId)g_machine->ReadIntRegister(4);


  //on stocke notre objet quand on le cherche, on recupere le semaphore de g_object_ids
  Semaphore * pointeurDeSemaphore;
  pointeurDeSemaphore = (Semaphore *)g_object_ids->SearchObject(reg_sema);
  ...
  case SC_LOCK_CREATE:
    ...
    //création du lock
    Lock *lock;
    lock = new Lock(nomParam);

    //on ajoute le lock
    int32_t ajoutLockId;
    ajoutLockId = g_object_ids->AddObject(lock);
  ...
  LockId reg_lock_id;
  reg_lock_id=(LockId)g_machine->ReadIntRegister(4);

  //on stocke notre objet quand on le cherche, on recupere le lock id
  Lock * pointeurDeLock;
  pointeurDeLock = (Lock *)g_object_ids->SearchObject(reg_lock_id);
  ...
  {
    //message debug
    DEBUG('e', (char *)"System call: Use of SC_COND_CREATE().\n");
    //recupere le nom du cond en r4
    int32_t reg_debug_name;
    reg_debug_name=g_machine->ReadIntRegister(4);
    //utilisation de GetStringParam et GetLengthParam pour recuperer le contenu de la chaine de caractere reg_debug_name et la taille de reg_count
    int tailleDuParam;
    ...
    //création du cond
    Condition * condition;
    condition= new Condition(nomParam);

    //on ajoute la condition
    int32_t ajoutConditionId;
    ajoutConditionId = g_object_ids->AddObject(condition);

    //si l'id mise en paramètres est valide alors pas d'erreur
    g_machine->WriteIntRegister(2,ajoutConditionId);
    g_syscall_error->SetMsg((char*)"",NO_ERROR);
  ...
  {
    //message debug
    DEBUG('e', (char *)"System call: Use of SC_COND_DESTROY().\n");
    //recupere le cond en r4
    CondId reg_cond_id;
    reg_cond_id=(CondId)g_machine->ReadIntRegister(4);

    //on stocke notre objet quand on le cherche, on recupere le cond id
    Condition * pointeurDeCondition;
    pointeurDeCondition = (Condition *)g_object_ids->SearchObject(reg_cond_id);

    //si l'objet correspondant à une cond on supprime
    if(pointeurDeCondition->type == CONDITION_TYPE){
        //on supprime le cond de g_object_ids
        g_object_ids->RemoveObject(reg_cond_id);

        //et on peut supprimer le cond
        pointeurDeCondition->~Condition();
        ...
        //Si l'objet n'est pas de type conditions on retourne l'erreur disant que l'id mise en paramètres est mauvais
        g_machine->WriteIntRegister(2, ERROR);
  ...
  {
    //message debug
    DEBUG('e', (char *)"System call: Use of SC_COND_WAIT().\n");
    //recupere le cond en r4
    CondId reg_cond_id;
    reg_cond_id=(CondId)g_machine->ReadIntRegister(4);
    ...
    //si l'objet correspondant à une cond on supprime
    if(pointeurDeCondition->type == CONDITION_TYPE){
        //on wait la condition
        pointeurDeCondition->Wait();
        ...
        //Si l'objet n'est pas de type condition on retourne l'erreur disant que l'id mise en paramètres est mauvais
        g_machine->WriteIntRegister(2, ERROR);
        g_syscall_error->SetMsg((char*)"",INVALID_CONDITION_ID);
      }
  ...
  break;
    ...
    {
    //message debug
    DEBUG('e', (char *)"System call: Use of SC_COND_SIGNAL().\n");
    //recupere le cond en r4
    CondId reg_cond_id;
    reg_cond_id=(CondId)g_machine->ReadIntRegister(4);
    ...
    //si l'objet correspondant à une cond on supprime
    if(pointeurDeCondition->type == CONDITION_TYPE){
        //et on envoie le signal
        pointeurDeCondition->Signal();
        ...
        //Si l'objet n'est pas de type cond on retourne l'erreur disant que l'id mise en paramètres est mauvais
        g_machine->WriteIntRegister(2, ERROR);
        g_syscall_error->SetMsg((char*)"",INVALID_CONDITION_ID);
      }
  ...
  {
  //message debug
  DEBUG('e', (char *)"System call: Use of SC_COND_BROADCAST().\n");
  //recupere le cond en r4
  CondId reg_cond_id;
  reg_cond_id=(CondId)g_machine->ReadIntRegister(4);
